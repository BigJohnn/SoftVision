...
#define BITMASK(Bitmask)                                            \
                                                                          \
  inline constexpr Bitmask operator| (Bitmask x , Bitmask y )       \
  { return static_cast<Bitmask>( static_cast<int_least32_t>(x)     \
      | static_cast<int_least32_t>(y)); }                          \
                                                                          \
  inline constexpr Bitmask operator& (Bitmask x , Bitmask y )       \
  { return static_cast<Bitmask>( static_cast<int_least32_t>(x)     \
      & static_cast<int_least32_t>(y)); }                          \
...
//    split(intrinsicsVec, str, is_any_of(joinChar));
    
    std::istringstream f(str);
    std::string s;
    while (getline(f, s, joinChar[0])) {
        intrinsicsVec.push_back(s);
    }
