    ...
    : Fuser(*mp_)
{
    ...
    doFilterOctreeTracks = true;//_mp.userParams.get<bool>("LargeScale.doFilterOctreeTracks", true);
    doUseWeaklySupportedPoints = false;//_mp.userParams.get<bool>("LargeScale.doUseWeaklySupportedPoints", false);
    doUseWeaklySupportedPointCam = false;//_mp.userParams.get<bool>("LargeScale.doUseWeaklySupportedPointCam", false);
    minNumOfConsistentCams = 2;//_mp.userParams.get<int>("filter.minNumOfConsistentCams", 2);
    simWspThr = -0.0f;//(float)_mp.userParams.get<double>("LargeScale.simWspThr", -0.0f);
...
//        tag::median(with_p_square_quantile)> > Accumulator;
    std::vector<float> accMinPixSize;
    std::vector<float> accMinSim;
    std::vector<float> accNbCamsA;
    std::vector<float> accNbCamsB;
    ...
    for(int i = 0; i < tracks->size(); i++)
    {
        ...
        accNbCamsA.emplace_back(t->cams.size());
//        accNbCamsA(t->cams.size());
        if(t->cams.size() >= minNumOfConsistentCams)
        {
            tracksOut.push_back((*tracks)[i]);
            
            accMinPixSize.emplace_back(t->minPixSize);
            accMinSim.emplace_back(t->minSim);
            accNbCamsB.emplace_back(t->cams.size());
...
//            accNbCamsB(t->cams.size());
    ...
    ALICEVISION_LOG_INFO("filterMinNumConsistentCams:  TODO...");
//                         << std::endl
...
void OctreeTracks::filterOctreeTracks2(StaticVector<trackStruct*>* tracks)
{
    ...
    float clusterSizeThr = 2.0f;//_mp.userParams.get<double>("OctreeTracks.clusterSizeThr", 2.0f);
