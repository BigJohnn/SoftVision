...
StaticVector<StaticVector<int>*>* loadLargeScalePtsCams(const std::vector<std::string>& recsDirs)
{
    ...
    for(int i = 0; i < recsDirs.size(); ++i)
    {
        ...
        if (!utils::exists(filePtsCamsFromDCTName))
        {
...
mesh::Mesh* joinMeshes(int gl, LargeScale* ls)
{
    ...
    int gridLevel = gl * 300;//ls->mp->userParams.get<int>(param.c_str(), gl * 300);
    ...
    std::string subFolderName = "";//ls->mp->userParams.get<std::string>("LargeScale.subFolderName", "");
