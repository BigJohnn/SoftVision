...
// #include <fuseCut/MaxFlow_CSR.hpp>
#include <fuseCut/MaxFlow_AdjList.hpp>
...
void filterByPixSize(const std::vector<Point3d>& verticesCoordsPrepare, std::vector<double>& pixSizePrepare, double pixSizeMarginCoef, std::vector<float>& simScorePrepare)
{
    ...
    for(int vIndex = 0; vIndex < verticesCoordsPrepare.size(); ++vIndex)
    {
        ...
        static const nanoflann::SearchParameters searchParams(0, false); // false: dont need to sort
  ...
  : _mp(mp)
{
    ...
    saveTemporaryBinFiles = false;//_mp.userParams.get<bool>("LargeScale.saveTemporaryBinFiles", false);
...
void DelaunayGraphCut::addGridHelperPoints(int helperPointsGridSize, const Point3d voxel[8], float minDist)
{
    ...
    const unsigned int seed = 0;//(unsigned int)_mp.userParams.get<unsigned int>("delaunaycut.seed", 0);
...
void DelaunayGraphCut::computeVerticesSegSize(std::vector<GC_Seg>& out_segments, const std::vector<bool>& useVertex, float alpha) // allPoints=true, alpha=0
{
    ...
    int scalePS = 1;//_mp.userParams.get<int>("global.scalePS", 1);
    int step = 1;//_mp.userParams.get<int>("global.step", 1);
    float pointToJoinPixSizeDist = 2.0/*(float)_mp.userParams.get<double>("delaunaycut.pointToJoinPixSizeDist", 2.0)*/ *
                                                      ...
                                                      // labatutWeights=0 fillOut=1 distFcnHeight=0
{
    ...
    auto progressDisplay =
            system2::createConsoleProgressDisplay(std::min(size_t(100), verticesRandIds.size()),
...
void DelaunayGraphCut::forceTedgesByGradientIJCV(float nPixelSizeBehind)
{
    ...
    const float forceTEdgeDelta = 0.1f;//(float)_mp.userParams.get<double>("delaunaycut.forceTEdgeDelta", 0.1f);
    ALICEVISION_LOG_DEBUG("forceTEdgeDelta: " << forceTEdgeDelta);

    const float minJumpPartRange = 10000.0f;//(float)_mp.userParams.get<double>("delaunaycut.minJumpPartRange", 10000.0f);
    ALICEVISION_LOG_DEBUG("minJumpPartRange: " << minJumpPartRange);

    const float maxSilentPartRange = 100.0f;//(float)_mp.userParams.get<double>("delaunaycut.maxSilentPartRange", 100.0f);
    ALICEVISION_LOG_DEBUG("maxSilentPartRange: " << maxSilentPartRange);

    const float nsigmaJumpPart = 4.0f;//(float)_mp.userParams.get<double>("delaunaycut.nsigmaJumpPart", 4.0f);
    ALICEVISION_LOG_DEBUG("nsigmaJumpPart: " << nsigmaJumpPart);

    const float nsigmaFrontSilentPart = 2.0f;//(float)_mp.userParams.get<double>("delaunaycut.nsigmaFrontSilentPart", 2.0f);
    ...
    const float nsigmaBackSilentPart = 2.0f;//(float)_mp.userParams.get<double>("delaunaycut.nsigmaBackSilentPart", 2.0f);
...
void DelaunayGraphCut::exportDebugMesh(const std::string& filename, const Point3d& fromPt, const Point3d& toPt)
{
    ...
    const std::string tempDirPath = utils::temp_directory_path();
...
{
    assert(utils::GetFileExtension(filePath) == std::string(".csv"));
