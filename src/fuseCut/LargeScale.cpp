...
{
    utils::create_directory(spaceFolderName);
    utils::create_directory(spaceVoxelsFolderName);

    doVisualize = false;//mp->userParams.get<bool>("LargeScale.doVisualizeOctreeTracks", false);
}
...
{
    return utils::exists(spaceFileName);
}
...
bool LargeScale::generateSpace(int maxPts, int ocTreeDim, bool generateTracks)
{
    ...
    if(generateTracks)
    {
        bool addRandomNoise = false;//mp->userParams.get<bool>("LargeScale.addRandomNoise", false);
        float addRandomNoisePercNoisePts = 10.0f;
//            (float)mp->userParams.get<double>("LargeScale.addRandomNoisePercNoisePts", 10.0);
        int addRandomNoiseNoisPixSizeDistHalfThr = 10.0f;
