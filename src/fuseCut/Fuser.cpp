...
#include <mvsData/Pixel.hpp>
#include <mvsData/Point2d.hpp>
#include <mvsData/Stat3d.hpp>
...
//    Accumulator accZ2( tag::tail<right>::cache_size = cacheSize );
    std::vector<float> accX1;
    std::vector<float> accX2;
    std::vector<float> accY1;
    std::vector<float> accY2;
    std::vector<float> accZ1;
    std::vector<float> accZ2;
    ...
    for(int rc = 0; rc < _mp.ncams; ++rc)
    {
        ...
        for(int i = 0; i < depthMap.size(); i += stepPts)
        {
            ...
            if(depth > 0.0f)
            {
                ...
                if(d1 < 0)
                    accX1.emplace_back(fabs(d1));//accX1(fabs(d1));
                else
                    accX2.emplace_back(fabs(d1));//accX2(fabs(d1));
                if(d2 < 0)
                    accY1.emplace_back(fabs(d2));//accY1(fabs(d2));
                else
                    accY2.emplace_back(fabs(d2));//accY2(fabs(d2));
                if(d3 < 0)
                    accZ1.emplace_back(fabs(d3));//accZ1(fabs(d3));
                else
                    accZ2.emplace_back(fabs(d3));//accZ2(fabs(d3));
            }
        }
        //mvsUtils::printfEstimate(rc, _mp.ncams, t1);
    }
    ...
    float perc = 0.999f;//(float)_mp.userParams.get<double>("LargeScale.universePercentile", 0.999f);

    float mind1 = -utils::quantile<float>(accX1, perc, cacheSize);
    float maxd1 = utils::quantile<float>(accX2, perc, cacheSize);
    float mind2 = -utils::quantile<float>(accY1, perc, cacheSize);
    float maxd2 = utils::quantile<float>(accY2, perc, cacheSize);
    float mind3 = -utils::quantile<float>(accZ1, perc, cacheSize);
    float maxd3 = utils::quantile<float>(accZ2, perc, cacheSize);
...
void Fuser::divideSpaceFromSfM(const sfmData::SfMData& sfmData, Point3d* hexah, std::size_t minObservations, float minObservationAngle) const
{
  ...
  const std::size_t cacheSize =  10000;
  const double percentile = 0.999;//_mp.userParams.get<double>("LargeScale.universePercentile", 0.999);
...
//  AccumulatorMax accMaxZ(tag::tail<right>::cache_size = cacheSize);
    std::vector<float> accMinX;
    std::vector<float> accMinY;
    std::vector<float> accMinZ;
    std::vector<float> accMaxX;
    std::vector<float> accMaxY;
    std::vector<float> accMaxZ;
  ...
  for(const auto& landmarkPair : sfmData.getLandmarks())
  {
    ...
    accMinX.emplace_back(x);
    accMinY.emplace_back(y);
    accMinZ.emplace_back(z);
    accMaxX.emplace_back(x);
    accMaxY.emplace_back(y);
    accMaxZ.emplace_back(z);
  }
...
//  double zMax = quantile(accMaxZ, quantile_probability = percentile);
    double xMin = utils::quantile<float>(accMinX, 1.0 - percentile, cacheSize, false);
    double yMin = utils::quantile<float>(accMinY, 1.0 - percentile, cacheSize, false);
    double zMin = utils::quantile<float>(accMinZ, 1.0 - percentile, cacheSize, false);
    double xMax = utils::quantile<float>(accMaxX, percentile, cacheSize);
    double yMax = utils::quantile<float>(accMaxY, percentile, cacheSize);
    double zMax = utils::quantile<float>(accMaxZ, percentile, cacheSize);
...
Voxel Fuser::estimateDimensions(Point3d* vox, Point3d* newSpace, int scale, int maxOcTreeDim, const sfmData::SfMData* sfmData)
{
    ...
    const float pointToJoinPixSizeDist = 2.0f;//(float)_mp.userParams.get<double>("Fuser.pointToJoinPixSizeDist", 2.0f);
                                     ...
                                     int noisPixSizeDistHalfThr)
{
    ...
    if (!utils::is_directory(depthMapsPtsSimsTmpDir))
    {
        utils::create_directory(depthMapsPtsSimsTmpDir);
...
void deleteTempPtsSimsFiles(mvsUtils::MultiViewParams& mp, const std::string& depthMapsPtsSimsTmpDir)
{
    ...
    utils::remove_all(depthMapsPtsSimsTmpDir);
}
