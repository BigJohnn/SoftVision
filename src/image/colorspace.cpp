...
//        {
            LOG_DEBUG("ALICEVISION_OCIO is empty.");
//        }
//        else
//        {
...
void initColorConfigOCIO(const std::string& colorConfigFilePath)
{
    ...
    {
        LOG_ERROR("OpenImageIO has not been compiled with OCIO.");
    }
    const std::string error = colorConfigOCIO.geterror();
    if (!error.empty())
    {
        LOG_ERROR("Erroneous OCIO config file %s:%s",colorConfigFilePath.c_str(), error.c_str());
    }
    ...
    LOG_ERROR("OCIO color config initialized with OCIO version: %d.%d.%d", ocioMajor, ocioMinor, ocioPatch);
}
...
//    const std::string type = boost::to_lower_copy(dataType);
    std::string type;
    std::transform(dataType.begin(), dataType.end(), type.begin(),
