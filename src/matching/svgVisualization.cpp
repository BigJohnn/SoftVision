                           ...
                           const std::string &outputSVGPath)
{
  ...
  if (!svgFile.is_open())
  {
    LOG_INFO("Unable to open file %s",outputSVGPath.c_str());
    return;
  }
  svgFile << svgStream.closeSvgFile().str();
  if (!svgFile.good())
  {
    LOG_INFO("Something wrong while writing file ",outputSVGPath.c_str());
    return;
  }
  svgFile.close();
}
                           ...
                           const std::string& outFilename)
{
  ...
  if(!svgFile.is_open())
  {
    LOG_INFO("Unable to open file %s",outFilename.c_str());
    return;
  }
  svgFile << svgStream.closeSvgFile().str();
  if(!svgFile.good())
  {
    LOG_INFO("Something wrong happened while writing file %s",outFilename.c_str());
    return;
  }
  svgFile.close();
}
                             ...
                             bool richKeypoint)
{
  ...
  if(!svgFile.good())
  {
    LOG_INFO("Something wrong while writing file %s" , outputSVGPath.c_str());
    return;
  }
  svgFile.close();
}
                              ...
                              bool left)
{
  ...
  for(const matching::IndMatch &m : matches)
  {
    ...
    else
    {
        {
            std::stringstream ss;
            ss<<"********** pts size: " << pts.size() << " epiline " << epiLine << " out of image"<<std::endl;
