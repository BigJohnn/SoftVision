...
#include <robustEstimation/ransacTools.hpp>
#include <robustEstimation/IRansacKernel.hpp>
#include <limits>
#include <numeric>
#include <iostream>
#include <vector>
                                       ...
                                       bool verbose = false)
{
  ...
  if(inliers.size() < min_samples)
  {
    ...
    if(verbose)
    {
      LOG_INFO("[IRLS] returning cause inliers.size() < min_samples");
    }
  ...
  if (models.size() == 0) 
  {
    ...
    if(verbose)
    {
      LOG_INFO("[IRLS] returning cause models.size() == 0");
    }
    return std::numeric_limits<double>::infinity();
  }
                                  ...
                                  double outliers_probability = 1e-2)
{
  ...
  for(iteration = 0; iteration < max_iterations; ++iteration) 
  {
    ...
    for(std::size_t i = 0; i < models.size(); ++i) 
    {
      ...
      if(bVerbose)
      {
        LOG_DEBUG("sample=%p", sample.data());
        LOG_DEBUG("model %lu e: %f",i, score);
      }
      ...
      if (bestNumInliers <= inliers.size()) 
      {
        ...
        if(bVerbose)
        {
          LOG_DEBUG("Before Optim: num inliers: %lu score %f kernel minimum nb required samples LS: %lu ", inliers.size(), score,kernel.getMinimumNbRequiredSamplesLS()
                 );

          LOG_DEBUG("Model:\n %p todo" , &bestModel.getMatrix());
        }
        ...
        if(bVerbose)
        {
          LOG_DEBUG("After Optim: num inliers: %lu score %f", inliers.size(), score);
          LOG_DEBUG("Model:\n %p todo" , &bestModel.getMatrix());
        }
