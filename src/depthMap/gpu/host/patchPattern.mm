...
void buildCustomPatchPattern(const CustomPatchPatternParams& patchParams)
{
    ...
    id<MTLBuffer> devicePatchPatternBuffer = [device newBufferWithLength:sizeof(DevicePatchPattern) options:MTLStorageModeShared];
    // host-side patch pattern pointer
    auto* patchPattern_h = (DevicePatchPattern*)devicePatchPatternBuffer.contents;
    ...
    if(patchParams.groupSubpartsPerLevel)
    {
        ...
        for(const auto& subpartParams : patchParams.subpartsParams)
        {
            ...
            if(subpartParams.isCircle)
            {
                ...
                {
                    auto& coords = subpart.coordinates[subpart.nbCoordinates + i];
    ...
    else
    {
        for(int i = 0; i < patchPattern_h->nbSubparts; ++i)
        {
            ...
            if(subpartParams.isCircle)
            {
                ...
                {
                    auto& coords = subpart.coordinates[j];
    ...
    // log patch pattern
    {
        ...
        for(int i = 0; i < patchPattern_h->nbSubparts; ++i)
        {
            ...
            if(subpart.isCircle)
            {
                ...
                for(int j = 0; j < subpart.nbCoordinates; ++j)
                {
                    const auto& coords = subpart.coordinates[j];
