...
//    float deltaP = __fsqrt_rn(float(dx * dx + dy * dy)); //CHECK
    ...
    // return exp(-(deltaC * deltaC / (2 * gammaC * gammaC))) * sqrtf(exp(-(deltaP * deltaP / (2 * gammaP * gammaP)))); // DCB
    // return exp(-((deltaC * deltaC / 2) * (invGammaC * invGammaC))) * sqrtf(exp(-(((deltaP * deltaP / 2) * (invGammaP * invGammaP)))); // DCB
}
 ...
 * @return distance value
 */
 inline float CostYKfromLab(const float4 c1, const float4 c2, const float invGammaC)
{
    // euclidean distance in Lab, assuming linear RGB
