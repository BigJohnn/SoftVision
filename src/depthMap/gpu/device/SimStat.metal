...
//    device simStat& operator=(device const simStat& param)
//    {
//        xxsum = param.xxsum;
//        yysum = param.yysum;
//        xysum = param.xysum;
...
//    }
    ...
    float computeSimSub(device simStat& ss)
    {
        // NCC
    ...
    */
    ...
    void update(device const float2& g, float w)
    {
        wsum += w;
        count += 1.0f;
        xsum += w * g.x;
    ...
    void update(device const float3& c1, device const float3& c2)
    {
        float2 g;
        g.x = c1.x;
        g.y = c2.x;
