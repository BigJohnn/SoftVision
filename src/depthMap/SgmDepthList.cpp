...
{
    LOG_X(_tile << "Compute SGM depths list.");
    ...
    if(nbObsDepths < 2)
    {
       LOG_X(_tile << "Cannot get min/max/middle depth from SfM.");
    ...
    {   
        ...
        {
            LOG_X(_tile << "Not enough valid samples over the epipolar line. Compute depth list from R camera pixel size.");
    ...
    {
        LOG_X(_tile << "No depths found.");
        return; // nothing to do
    }

    LOG_X(_tile << "Depth candidates from seeds for R camera:" << std::endl
    ...
    if(_sgmParams.useSfmSeeds && !_mp.getInputSfMData().getLandmarks().empty() && nbObsDepths > 10)
    {
        ...
        LOG_X(_tile << "Final depth range (intersection: frustums / landmarks with margin): [" << firstDepth << "-" << lastDepth << "]");
    }
    ...
    if(_sgmParams.maxDepths > 0 && _depths.size() > _sgmParams.maxDepths)
    {
        ...
        LOG_X(_tile << "Too many values in R camera depth list, filter out with scale factor:" << std::endl
    ...
    LOG_X(_tile << "Final depth range for R camera:" << std::endl
    ...
    LOG_X(_tile << "Compute SGM depths list done.");
}
...
void SgmDepthList::removeTcWithNoDepth(Tile& tile) 
{
    ...
    for(size_t c = 0; c < tile.sgmTCams.size(); ++c)
    {
        ...
        {
            LOG_X(_tile << "Remove T camera (tc: " << tc << ", view id: " << _mp.getViewId(tc) << ") no depth found.");
        }
    }
                                              ...
                                              std::size_t& out_nbDepths) const
{
    ...
    LOG_X(_tile << "Compute min/max/mid/nb observation depth from SfM for R camera:" << std::endl
                                            ...
                                            double& out_zmax) const
{
    ...
    LOG_X(_tile << "Compute min/max depth from common Rc/Tc SfM observations:" << std::endl
                                     ...
                                     std::vector<float>& out_depths) const
{
    ...
    LOG_X(_tile << "Find depths over the epipolar line segment between R and T cameras:" << std::endl
    ...
    if(!out_depths.empty())
      LOG_X(_tile << "Depth to use range [" << out_depths.front() << "-" << out_depths.back() << "]" << std::endl);
}
