...
void Sgm::computeSimilarityVolumes(const Tile& tile, const SgmDepthList& tileDepthList)
{
    ...
    volumeInitialize(_volumeBestSim_dmp, TSim(255.f));
    volumeInitialize(_volumeSecBestSim_dmp, TSim(255.f));
...
//    volumeSim_hmh.copyFrom(in_volume_dmp);
    DeviceBuffer* volumeSim_hmh = in_volume_dmp;//[DeviceBuffer allocate:[in_volume_dmp getSize] elemSizeInBytes:sizeof(TSim) elemType:@"TSim"];
