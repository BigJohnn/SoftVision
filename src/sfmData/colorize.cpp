...
{
  auto progressDisplay = system2::createConsoleProgressDisplay(sfmData.getLandmarks().size(), std::cout,
...
#pragma omp parallel for
  for(int i = 0; i < unsortedIndexes.size(); ++i)
  {
    ...
    if(!viewCardinal.landmarks.empty())
    {
      ...
      image::Image<image::RGBAColor> image;
//      image::readImage(view.getImagePath(), image, image::EImageColorSpace::SRGB);
        image::byteBuffer2EigenMatrix(view.getWidth(), view.getHeight(), view.getBuffer(), image);
      ...
      for(Landmark& landmark : viewCardinal.landmarks)
      {
        ...
        pt.y() = clamp(pt.y(), 0.0, static_cast<double>(image.Height() - 1));
          ...
          auto&& rgba = image(pt.y(), pt.x());
        landmark.rgb = image::RGBColor(rgba.r(), rgba.g(), rgba.b());
      }
