                                 ...
                                 image::EImageFileType textureFileType)
{
    ...
    system2::MemoryInfo memInfo = system2::getMemoryInfo();
...
#if TEXTURING_MBB_DEBUG
        {
            ...
            if(!texParams.useScore)
            {
                for(std::size_t level = 0; level < accuPyramid.pyramid.size(); ++level)
                {
                    ...
                    writeImage(texturePath, texParams.textureSide, texParams.textureSide, imgContrib, EImageQuality::OPTIMIZED, colorspace);
                }
            }
        }
#endif
...
void Texturing::generateNormalAndHeightMaps(const mvsUtils::MultiViewParams& mp, const Mesh& denseMesh,
                                            const std::string& outPath, const mesh::BumpMappingParams& bumpMappingParams)
{
...
void Texturing::writeTexture(AccuImage& atlasTexture, const std::size_t atlasID, const std::string &outPath,
                             image::EImageFileType textureFileType, const int level)
{
    ...
    std::string texturePath = outPath + textureName; //TODO: check
    ALICEVISION_LOG_INFO("  - Writing texture file: " << texturePath);

    image::writeImage(texturePath, atlasTexture.img,
...
void Texturing::unwrap(mvsUtils::MultiViewParams& mp, EUnwrapMethod method)
{
    ...
    else
    {
        ...
        // and reload it
        const std::string tmpObjPath = (utils::temp_directory_path() + utils::random_string()) + ".obj";
        // save temp mesh with UVs
        GEO::mesh_save(geoMesh, tmpObjPath);
        // replace initial mesh
        replaceMesh(tmpObjPath);
        // remove temp mesh
        utils::remove(tmpObjPath);
    }
}

void Texturing::saveAs(const std::string& dir, const std::string& basename, EFileType meshFileType)
{
    const std::string meshFileTypeStr = EFileType_enumToString(meshFileType);
    const std::string filepath = (dir + (basename + "." + meshFileTypeStr));
...
#pragma omp parallel for
    ...
    if(bumpMappingParams.bumpType == EBumpMappingType::Normal && bumpMappingParams.bumpMappingFileType != image::EImageFileType::NONE)
    {
        ...
        std::string normalMapPath = outPath + name; //TODO: check
        ALICEVISION_LOG_INFO("Writing normal map: " << normalMapPath);

        image::writeImage(normalMapPath, normalMap,
    ...
    if(bumpMappingParams.bumpMappingFileType != image::EImageFileType::NONE || bumpMappingParams.displacementFileType != image::EImageFileType::NONE)
    {
        ...
        if(bumpMappingParams.bumpType == EBumpMappingType::Height)
        {
            ...
            std::string bumpMapPath = outPath + bumpName;//TODO: check
            ALICEVISION_LOG_INFO("Writing bump map: " << bumpMapPath);

            image::writeImage(bumpMapPath, heightMap,
        ...
        if(bumpMappingParams.displacementFileType != image::EImageFileType::NONE)
        {
            ...
            std::string dispMapPath = outPath + dispName; //TODO: check
            ALICEVISION_LOG_INFO("Writing displacement map: " << dispMapPath);

            image::writeImage(dispMapPath, heightMap,
                              image::ImageWriteOptions().storageDataType(image::EStorageDataType::Half));
        }
    }
}
