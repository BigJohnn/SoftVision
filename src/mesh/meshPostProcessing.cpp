...
#include <mesh/MeshEnergyOpt.hpp>
                      ...
                      StaticVector<Point3d>* hexahsToExcludeFromResultingMesh, Point3d* hexah)
{
    ...
    bool exportDebug = false;//(float)mp.userParams.get<bool>("delaunaycut.exportDebugGC", false);
    ...
    // copy ptsCams
    {
        ...
        bool doRemoveTrianglesInhexahsToExcludeFromResultingMesh = false;//
...
//                                       false);
    ...
    if(true) // TODO: how to remove it?
    {
        ...
        /////////////////////////////
        bool doSmoothMesh = true;
//            mp.userParams.get<bool>("meshEnergyOpt.doSmoothMesh", true);
        int smoothNIter = 0;//mp.userParams.get<int>("meshEnergyOpt.smoothNbIterations", 0);
        if(doSmoothMesh && smoothNIter != 0)
        {
            ALICEVISION_LOG_INFO("Mesh smoothing.");
            float lambda = 1.0f;//(float)mp.userParams.get<double>("meshEnergyOpt.lambda", 1.0f);
