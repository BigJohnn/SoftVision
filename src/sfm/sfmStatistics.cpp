...
void computeResidualsHistogram(const sfmData::SfMData& sfmData, BoxStats<double>& out_stats, utils::Histogram<double>* out_histogram, const std::set<IndexT>& specificViews)
{
  ...
  {
    const sfmData::Observations & observations = track.second.observations;
    for(const auto& obs: observations)
    {
      ...
      const std::shared_ptr<camera::IntrinsicBase> intrinsic = sfmData.getIntrinsics().find(view.getIntrinsicId())->second;
      const Vec2 residual = intrinsic->residual(pose, track.second.X.homogeneous(), obs.second.x);
      vec_residuals.push_back(residual.norm());
      //LOG_INFO("[AliceVision] sfmtstatistics::computeResidualsHistogram track: " << track.first << ", residual: " << residual.norm());
    }
  }

 // LOG_INFO("[AliceVision] sfmtstatistics::computeResidualsHistogram vec_residuals.size(): " << vec_residuals.size());
