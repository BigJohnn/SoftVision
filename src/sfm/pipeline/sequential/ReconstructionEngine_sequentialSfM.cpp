...
#include <sfm/utils/statistics.hpp>
...
#include <sfm/sfmStatistics.hpp>

#include <feature/FeaturesPerView.hpp>
#include <graph/connectedComponent.hpp>
#include <matching/IndMatch.hpp>
#include <multiview/essential.hpp>
#include <multiview/triangulation/triangulationDLT.hpp>
...
std::size_t ReconstructionEngine_sequentialSfM::fuseMatchesIntoTracks()
{
...

  {
    ...
    const matching::PairwiseMatches& matches = *_pairwiseMatches;

    LOG_X("Track building");
    tracksBuilder.build(matches);

    LOG_X("Track filtering");
    tracksBuilder.filter(_params.filterTrackForks, _params.minInputTrackLength);

    LOG_X("Track export to internal structure");
    // build tracks with STL compliant type
    tracksBuilder.exportToSTL(_map_tracks);
    LOG_X("Build tracks per view");
    ...
    LOG_X("Build tracks pyramid per view");
    ...
    // display stats
    {
      ...
      LOG_X("Fuse matches into tracks: " << std::endl
      ...
      LOG_INFO("TrackLength, Occurrence");
...
void ReconstructionEngine_sequentialSfM::createInitialReconstruction(const std::vector<Pair>& initialImagePairCandidates)
{
  ...
  for(const auto& initialPairCandidate: initialImagePairCandidates)
  {
    if(makeInitialPair3D(initialPairCandidate))
    {
      ...
      LOG_X("Initial pair is: " << initialPairCandidate.first << ", " << initialPairCandidate.second);
...
void ReconstructionEngine_sequentialSfM::remapLandmarkIdsToTrackIds()
{
  ...
  LOG_X("Builds landmarks temporary comparison structure");
  ...
  LOG_X("Find corresponding landmark id per track id");
  ...
  LOG_X("Landmark ids to track ids remapping: " << std::endl
...
double ReconstructionEngine_sequentialSfM::incrementalReconstruction()
{
  ...
  do
  {
    ...
    LOG_X("Incremental Reconstruction start iteration " << globalIteration << ":" << std::endl
    ...
    {
      LOG_X("Update Reconstruction:" << std::endl
...
//    {
//      LOG_INFO("Rig(s) calibration start");
//
//      std::set<IndexT> updatedViews;
//
...
//      bundleAdjustment(updatedViews);
//
//      LOG_INFO("Rig calibration finished:\n\t- # updated views: " << updatedViews.size());
//    }
    ++globalIteration;
  }
  while(nbValidPoses != _sfmData.getPoses().size());

  LOG_X("Incremental Reconstruction completed with " << globalIteration << " iterations:" << std::endl
                       << "\t- # number of resection groups: " << resectionId << std::endl
                       << "\t- # number of poses: " << nbValidPoses << std::endl
                       << "\t- # number of landmarks: " << _sfmData.structure.size() << std::endl
                       << "\t- # remaining images: " << remainingViewIds.size()
...
//      {
//        LOG_X("Resection of image " << i << " was skipped." << std::endl
...
#pragma omp critical
    {
      if(hasResected)
      {
        ...
        LOG_X("Resection of image " << i << " ( view id: " << viewId << " ) succeed.");
        _sfmData.getViews().at(viewId)->setResectionId(resectionId);
      }
      else
      {
        LOG_X("Resection of image " << i << " ( view id: " << viewId << " ) was not possible.");
      }
    }
  }

  LOG_X("Resection of " << bestViewIds.size() << " new images took " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - chrono_start).count() << " msec.");
...
void ReconstructionEngine_sequentialSfM::triangulate(const std::set<IndexT>& prevReconstructedViews, const std::set<IndexT>& newReconstructedViews)
{
  ...
  LOG_X("Triangulation of the " << newReconstructedViews.size() << " newly reconstructed views took " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - chrono_start).count() << " msec.");
}

bool ReconstructionEngine_sequentialSfM::bundleAdjustment(std::set<IndexT>& newReconstructedViews, bool isInitialPair)
{
  LOG_INFO("Bundle adjustment start.");
...
//    if((nbRefinedPoses <= newReconstructedViews.size()) && _sfmData.getRigs().empty())
    if((nbRefinedPoses <= newReconstructedViews.size()))
    {
      LOG_X("Local bundle adjustment: the new cameras are not connected to the rest of the graph"
  ...
  do
  {
    LOG_X("Start bundle adjustment iteration: " << iteration);
  ...
  LOG_X("Bundle adjustment with " << iteration << " iterations took " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - chronoStart).count() << " msec.");
  return true;
}
...
void ReconstructionEngine_sequentialSfM::exportStatistics(double reconstructionTime)
{
  ...
  LOG_X("Structure from Motion statistics:" << std::endl
  ...
  {
    LOG_X(" - # " << EImageDescriberType_enumToString(d.first) << ": " << d.second);
  }
  ...
  utils::Histogram<double> residualHistogram;
  {
      ...
      LOG_X("Histogram of residuals:" << residualHistogram.ToString("", 2));
  }
  ...
  utils::Histogram<double> observationsLengthHistogram;
  {
      ...
      LOG_X("# landmarks: " << _sfmData.getLandmarks().size());
      LOG_X("# overall observations: " << overallNbObservations);
      LOG_X("Landmarks observations length min: " << observationsLengthStats.min << ", mean: " << observationsLengthStats.mean << ", median: " << observationsLengthStats.median << ", max: "  << observationsLengthStats.max);
      LOG_X("Histogram of observations length:" << observationsLengthHistogram.ToString("", 6));
  }
  ...
  utils::Histogram<double> landmarksPerViewHistogram;
  {
      ...
      LOG_X("Landmarks per view min: " << landmarksPerViewStats.min << ", mean: " << landmarksPerViewStats.mean << ", median: " << landmarksPerViewStats.median << ", max: " << landmarksPerViewStats.max);
      LOG_X("Histogram of nb landmarks per view:" << landmarksPerViewHistogram.ToString<int>("", 3));
  }
...
//    for(std::size_t i = 2; i < obsHistogram.size(); ++i)
    ...
    //_jsonLogTree.put("hardware.cpu.freq", system2::cpu_clock_by_os());        // cpu frequency
    //_jsonLogTree.put("hardware.cpu.cores", system2::get_total_cpus());        // cpu cores
    //_jsonLogTree.put("hardware.ram.size", system2::getMemoryInfo().totalRam); // ram size

    // write json on disk TODO: ..
...
#pragma omp parallel for
  for(int i = 0; i < remainingViewIds.size(); ++i)
  {
    ...
    const track::TrackIdSet& set_tracksIds = _map_tracksPerView.at(viewId);
  ...
  const std::set<IndexT>& remainingViewIds) const
{
  ...
  {
    LOG_X("FindConnectedViews does not find connected new views ");
    return false;
  }
  ...
  LOG_X("findNextBestViews -- Scores (features): ");
...
//  LOG_X << std::endl;
  ...
  {
    LOG_X("Failed to find next best views :");
    if(vec_viewsScore.empty())
    {
      LOG_X("No putative image.");
    }
    else
    {
...
//    LOG_X << std::endl;
     ...
     !out_selectedViewIds.empty())
  {
    ...
    LOG_X("findNextBestViews: beginning of the incremental SfM" << std::endl
...
bool ReconstructionEngine_sequentialSfM::makeInitialPair3D(const Pair& currentPair)
{
  ...
  {
    LOG_X("Can't find initial image pair intrinsics: " << viewI.getIntrinsicId() << ", "  << viewJ.getIntrinsicId());
    return false;
  }
  ...
  {
    LOG_X("Can't find initial image pair intrinsics (NULL ptr): " << viewI.getIntrinsicId() << ", "  << viewJ.getIntrinsicId());
    return false;
  }

  // b. get common features between the two views
  // use the track to have a more dense match correspondence set
  track::TracksMap commonTracks;
  ...
  for (track::TracksMap::const_iterator
  ...
  LOG_X(n << " matches in the image pair for the initial pose estimation.");
  ...
  {
    LOG_INFO("Robust estimation failed to compute E for this pair");
    return false;
  }

  LOG_X("A-Contrario initial pair residual: " << relativePoseInfo.found_residual_precision);
...
//        _sfmData.resetRigs();
    ...
    BoxStats<double> residualStats;
    computeResidualsHistogram(_sfmData, residualStats, &residualHistogram);
    LOG_X("MSE Residual initial pair inlier: " << residualStats.mean);
...
bool ReconstructionEngine_sequentialSfM::getBestInitialImagePairs(std::vector<Pair>& out_bestImagePairs, IndexT filterViewId) {
  ...
  {
    LOG_INFO("Failed to find an initial pair automatically. There is no view with valid intrinsics.");
    return false;
  }

  if(filterViewId != UndefinedIndexT)
    LOG_X("Selection of an initial pair with one given view id: " << filterViewId << ".");
  ...
  auto progressDisplay = system2::createConsoleProgressDisplay(_pairwiseMatches->size(), std::cout,
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < _pairwiseMatches->size(); ++i)
  {
    ...
    LOG_X("Automatic initial pair choice test - I: " << I << ", J: " << J << ", common tracks: " << n);
    Mat xI(2,n), xJ(2,n);
    size_t cptIndex = 0;
...
//    LOG_X(boost::format("%=25s | %+15.1f | %+15.1f | %+15.1f | %+15f") % pairIdx % std::get<0>(s) % std::get<1>(s) % std::get<2>(s) % std::get<3>(s));
  ...
  {
    LOG_ERROR("No valid initial pair found automatically.");
    return false;
  }
...
bool ReconstructionEngine_sequentialSfM::computeResection(const IndexT viewId, ResectionData& resectionData)
{
  ...
  if (resectionData.tracksId.empty())
  {
    ...
    LOG_X("Resection failed as there is no connection with already reconstructed points");
    return false;
  }
  ...
  // C. Do the resectioning: compute the camera pose.
  LOG_X("[" << _sfmData.getValidViews().size()+1 << "/" << _sfmData.getViews().size() << "] Robust Resection of view: " << viewId);
  ...
  if (!bResection)
  {
    LOG_X("Resection of view " << viewId << " failed.");
    return false;
  }
  ...
  // We use a local scene with only the 3D points and the new camera.
  {
    ...
    {
      LOG_X("Resection of view " << viewId << " failed during pose refinement.");
      return false;
    }
  }
  return true;
                                   ...
                                   IndexT viewId, const track::Track& track)
{
    ...
    if (!camPinHole) {
        LOG_ERROR("Camera is not pinhole in triangulate_multiViewsLORANSAC");
        return {nullptr, {}, {}, {}, {}};
    }
...
{
  LOG_X("Triangulating (mode: multi-view LO-RANSAC)... ");
...
//  {
//    LOG_X("--Triangulated 3D points [" << I << "-" << J << "]:\n"
...
std::size_t ReconstructionEngine_sequentialSfM::removeOutliers()
{
  ...
  LOG_X("Remove outliers: " << std::endl
