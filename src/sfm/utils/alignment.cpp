...
#include <sfm/utils/alignment.hpp>
#include <sfm/liealgebra.hpp>
#include <geometry/rigidTransformation3D.hpp>
#include <stl/regex.hpp>
    ...
    Vec3* out_t)
{
    ...
    {
        LOG_X("Cannot compute similarities without common view.");
        return false;
    }
    ...
    if (!geometry::ACRansac_FindRTS(xA, xB, randomNumberGenerator, S, t, R, inliers, true))
        return false;

    LOG_X("There are " << reconstructedCommonViewIds.size() << " common cameras and " << inliers.size() << " were used to compute the similarity transform.");
                       ...
                       Vec3* out_t)
{
    ...
    LOG_X("Found " << commonViewIds.size() << " common views.");
        ...
        Vec3* out_t)
{
    ...
    LOG_X("Found " << commonPoseIds.size() << " common views.");
    if (commonPoseIds.empty())
    {
        LOG_X("Cannot compute similarities without common pose.");
        return false;
    }
    ...
    LOG_X("There are " << commonPoseIds.size() << " common camera poses and " << inliers.size() << " were used to compute the similarity transform.");
    ...
    const std::string& filePatternMatching)
{
    ...
    for (auto& viewIt : sfmData.getViews())
    {
        ...
        LOG_X("retrieveMatchingFilepath: " << imagePath << " -> " << cumulatedValues);
    ...
    const std::vector<std::string>& metadataList)
{
    ...
    for (auto& viewIt : sfmData.getViews())
    {
        ...
        LOG_X("retrieveUniqueMetadataValues: " << viewIt.second->getImagePath() << " -> " << cumulatedValues);
    ...
    Vec3* out_t)
{
    ...
    LOG_X("Found " << commonMarkers.size() << " common markers.");
    if (commonMarkers.empty())
    {
        LOG_X("Cannot compute similarities without common marker.");
        return false;
    }
    ...
    LOG_X("There are " << commonLandmarks.size() << " common markers and " << inliers.size() << " were used to compute the similarity transform.");
                                           ...
                                           Vec3& out_t)
{
    ...
    LOG_X("computeNewCoordinateSystemFromCamerasXAxis: eigenvalues: " << solver.eigenvalues());
    LOG_X("computeNewCoordinateSystemFromCamerasXAxis: eigenvectors: " << solver.eigenvectors());
...
//    accumulator_set<double, stats<tag::mean>> accX, accY, accZ;
    ...
    {
        vecSum += pose.second.getTransform().center();
...
//    const Vec3 camerasCenter(extract::mean(accX), extract::mean(accY), extract::mean(accZ));
    const Vec3 camerasCenter  = vecSum / sfmData.getPoses().size();
...
void computeNewCoordinateSystemFromSingleCamera(const sfmData::SfMData& sfmData, const IndexT viewId, double& out_S, Mat3& out_R, Vec3& out_t)
{
    ...
    LOG_X("computeNewCoordinateSystemFromSingleCamera orientation: " << int(orientation));
    ...
    )
{
    ...
    for (int i = 0; i < landmarksIds.size(); ++i)
    {
        ...
        if (landmarkId == -1)
        {
            LOG_X("Failed to find marker: " << int(markers[i].id));
        }
    }
...
bool computeNewCoordinateSystemFromGpsData(const sfmData::SfMData& sfmData, std::mt19937 &randomNumberGenerator, double& out_S, Mat3& out_R, Vec3& out_t)
{
    ...
    for(const auto& v : sfmData.getViews())
    {
        ...
        {
            LOG_X("Skipping view " << viewID << " because pose " << sfmData.isPoseAndIntrinsicDefined(viewID) << " and gps " << view->hasGpsMetadata());
            continue;
        }
    ...
    {
        LOG_X("Not enough points to estimate the rototranslation to align the gps");
        return false;
    }
    ...
    return geometry::ACRansac_FindRTS(x1, x2, randomNumberGenerator, out_S, out_t, out_R, inliers, refine);
}
...
void computeNewCoordinateSystemGroundAuto(const sfmData::SfMData& sfmData, Vec3& out_t)
{
    ...
    if (points.empty())
    {
        LOG_X("Ground detection failed as there is no valid point");
        return;
    }
...
void computeNewCoordinateSystemAuto(const sfmData::SfMData& sfmData, double& out_S, Mat3& out_R, Vec3& out_t)
{
    ...
    LOG_X("X axis rotation:" << std::endl
            << out_R
            );
    ...
    if (list_pairs.empty())
    {
        LOG_X("No GPS information available to use it.");
        return;
    }

    if (list_pairs.size() < minimalGpsMeasuresCount)
    {
        LOG_X("Not enough GPS information available to use it "
    ...
    if (var < distVariance)
    {
        LOG_X("Scene is too small to use GPS information "
            "(dataset variance: " << var << ", min variance: " << distVariance << ").");
        return;
    }
        
    LOG_X("GPS point cloud scale: " << gpsCov.trace());
    LOG_X("Linked cameras centers scale: " << camCov.trace());
