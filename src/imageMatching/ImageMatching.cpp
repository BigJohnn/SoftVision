...
#include <voctree/databaseIO.hpp>
...
{
    //TODO: check
    std::vector<std::pair<IndexT, IndexT>> sortedImages;
    sortedImages.reserve(sfmData.getViews().size());
    for (const auto& vIt: sfmData.getViews())
    {
        sortedImages.emplace_back(vIt.second->getViewId(), vIt.first);
    }
    std::sort(sortedImages.begin(), sortedImages.end());
    for (size_t i = 0; i < sortedImages.size(); ++i)
    {
        for (size_t n = i+1, nMax = std::min(i+nbMatches+1, sortedImages.size()); n < nMax; ++n)
        {
            size_t a = sortedImages[i].second;
            size_t b = sortedImages[n].second;
...
{
    LOG_INFO("Generate matches in mode: %s" ,
                         EImageMatchingMode_enumToString(modeMultiSfM).c_str());
...
#pragma omp parallel for
    for (ptrdiff_t i = 0; i < static_cast<ptrdiff_t>(descriptorsFiles.size()); ++i)
    {
        ...
        voctree::SparseHistogram imageSH;
        ...
        std::vector<voctree::DocMatch> matches;
        ...
        for (const voctree::DocMatch& m : matches)
        {
                      ...
                      OrderedPairList& selectedPairs)
{
    ...
    LOG_INFO("Loading vocabulary tree");

    auto loadVoctree_start = std::chrono::steady_clock::now();
    voctree::VocabularyTree<DescriptorFloat> tree(treeName);
    ...
    LOG_INFO("Creating the databases...");

    // add each object (document) to the database
    voctree::Database db(tree.words());
    voctree::Database db2;
    ...
    {
        LOG_INFO("No weights specified, skipping...");
    }
    ...
    // read the descriptors and populate the databases
    {
        ...
        LOG_INFO("Reading descriptors from: \n%s" , ss.str().c_str());
        ...
        LOG_INFO("Read %d sets of descriptors for a total of %d features",nbSetDescriptors,(nbFeaturesLoadedInputA + nbFeaturesLoadedInputB));
        LOG_INFO("Reading took %d sec.",detect_elapsed.count());
    }

    if (!withWeights)
    {
        // compute and save the word weights
        LOG_INFO("Computing weights...");
...

    {
        ...
        auto detect_elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::steady_clock::now() - detect_start);
        LOG_INFO("Query all documents took %d sec.",detect_elapsed.count());

        // process pair list
        detect_start = std::chrono::steady_clock::now();

        LOG_INFO("Convert all matches to pairList");
        ...
        detect_elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::steady_clock::now() - detect_start);
        LOG_INFO("Convert all matches to pairList took %d sec.",detect_elapsed.count());
    }
}
                                               ...
                                               std::size_t minNbImages)
{
    if (method == EImageMatchingMethod::FRUSTUM_OR_VOCABULARYTREE)
    {
        ...
        {
            LOG_INFO("FRUSTUM_OR_VOCABULARYTREE: Use VOCABULARYTREE matching, as there is no known pose.");
        ...
        {
            LOG_INFO("FRUSTUM_OR_VOCABULARYTREE: Use FRUSTUM intersection from known poses.");
            method = EImageMatchingMethod::FRUSTUM;
        }
        else
        {
            LOG_ERROR("%lu reconstructed views for %lu views.",reconstructedViews,sfmDataA.getViews().size());
        ...
        method == EImageMatchingMethod::SEQUENTIAL_AND_VOCABULARYTREE)
    {
        ...
        {
            LOG_DEBUG("Use EXHAUSTIVE method instead of VOCABULARYTREE (less images than minNbImages).");
