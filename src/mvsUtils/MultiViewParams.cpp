    ...
    , _processDownscale(downscale)
{
    ...
    // load image uid, path and dimensions
    {
        ...
        for(const auto& viewPair : sfmData.getViews())
        {
          ...
          else if(_imagesFolder != "/" && !_imagesFolder.empty() && utils::is_directory(_imagesFolder) && !utils::is_empty(_imagesFolder))
          {
            ...
            std::vector<std::string> paths = utils::getFilesPathsFromFolder(_imagesFolder, 
                ...
                {
                    return (utils::GetFileNameStem(path) == std::to_string(view.getViewId()) &&
                            (image::isSupportedUndistortFormat(utils::GetFileExtension(path))));
                }
            );
...
//                    std::to_string(view.getViewId()) + "' in folder '" + _imagesFolder + "'.");
        ...
        LOG_X("Found " << dimensions.size() << " image dimension(s): ");
        for(const auto& dim : dimensions)
            LOG_X("\t- [" << dim.first << "x" << dim.second << "]");
    }
...
//                throw std::runtime_error("Scale of file: '" + imgParams.path + "' is not uniform, check image dimension ratio.");
        ...
        {
            LOG_X("Reading view " << getViewId(i) << " projection matrix from image metadata.");
        ...
        else
        {
            ...
            {
                LOG_X("Reading view " << getViewId(i) << " projection matrix from SfMData.");
        ...
        {
            LOG_X("Camera " << i << " at infinity. Setting to zero");
    ...
    LOG_X("Overall maximum dimension: [" << _maxImageWidth << "x" << _maxImageHeight << "]");
}
...
void MultiViewParams::loadMatricesFromSfM(int index)
{
  ...
  if (!ptrPinHole) {
    LOG_X("Camera is not pinhole in loadMatricesFromRawProjectionMatrix");
    return;
  }
...
StaticVector<int> MultiViewParams::findCamsWhichIntersectsHexahedron(const Point3d hexah[8]) const
{
    ...
    for(int rc = 0; rc < getNbCameras(); rc++)
    {
        ...
        {
            LOG_X("Cannot find min / max depth metadata in image: " << getImagePath(rc) << ". Assumes that all images should be used.");
            tcams.push_back(rc);
        }
        else
        {
