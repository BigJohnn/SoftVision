...
void ImagesCache<Image>::initIC( std::vector<std::string>& imagesNames )
{
    ...
    float maxmbCPU = 5000.0f; //TODO: choose a reasonable value
...
void ImagesCache<Image>::refreshData(int camId)
{
    ...
    if(_camIdMapId[camId] == -1)
    {
        ...
        LOG_X("Add " << imagePath << " to image cache. " << formatElapsedTime(t1));
    }
    else
    {
      LOG_X("Reuse " << _imagesNames.at(camId) << " from image cache. ");
    }
}
