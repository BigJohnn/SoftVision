               ...
               image::EImageColorSpace colorspace, ECorrectEV correctEV)
{
    ...
    else
    {
        ...
        if(exposureCompensation == -1)
        {
            ...
            LOG_X("Cannot compensate exposure. PrepareDenseScene needs to be update");
        }
        else
        {
            LOG_X("  exposure compensation for image " << camId + 1 << ": " << exposureCompensation);
    ...
    if(processScale > 1)
    {
        LOG_X("Downscale (x" << processScale << ") image: " << mp.getViewId(camId) << ".");
        Image bmpr;
