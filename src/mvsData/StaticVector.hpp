...
{
    LOG_X("[IO] saveArrayOfArraysToFile: " << fileName);
...
{
    LOG_X("[IO] loadArrayOfArraysFromFile: " << fileName);
...
void saveArrayToFile(const std::string& fileName, const StaticVector<T>* a, bool docompress = true)
{
    LOG_X("[IO] saveArrayToFile: " << fileName);

//    boost::filesystem::path filepath = fileName;
//    boost::filesystem::create_directories( filepath.parent_path() );
    utils::create_directory(utils::GetParentPath(fileName));

    if( !a )
    {
        LOG_X("[IO] saveArrayToFile called with NULL static vector");
        return;
    }

    if( a->empty() )
    {
        LOG_X("[IO] saveArrayToFile called with 0-sized static vector");
        return;
    }
    ...
    else
    {
        ...
        */
        ...
        if(err != Z_OK)
        {
            LOG_X("compress error " << err << " : " << (sizeof(T) * a->size()) << " -> " << comprLen << ", n " << a->size());
...
{
    LOG_X("[IO] loadArrayFromFile: " << fileName);
...
bool loadArrayFromFile( StaticVector<T>& out, const std::string& fileName, bool printfWarning = false)
{
    ...
    else
    {
        ...
        size_t retval = fread(&n, sizeof(int), 1, f);
        if( retval != 1 )
            LOG_X("[IO] loadArrayFromFile: can't read array size (1) from " << fileName);
        out.clear();
        ...
        if(n == -1)
        {
            retval = fread(&n, sizeof(int), 1, f);
            if( retval != 1 )
                LOG_X("[IO] loadArrayFromFile: can't read array size (2)");
            out.resize(n);

            uLong comprLen;
            retval = fread(&comprLen, sizeof(uLong), 1, f);
            if( retval != 1 )
                LOG_X("[IO] loadArrayFromFile: can't read ulong elem size");
            Byte* compr = (Byte*)calloc((uInt)comprLen, 1);
            retval = fread(compr, sizeof(Byte), comprLen, f);
            if( retval != comprLen )
                LOG_X("[IO] loadArrayFromFile: can't read blob");
            ...
            if(err != Z_OK)
            {
                LOG_X("uncompress error " << err << " : " << (sizeof(T) * n) << " -> " << uncomprLen << ", n " << n);
            }
        ...
        else
        {
            ...
            if( retval != n )
                LOG_X("[IO] loadArrayFromFile: can't read n elements");
        }
...
{
    LOG_X("[IO] loadArrayFromFileIntoArray: " << fileName);
