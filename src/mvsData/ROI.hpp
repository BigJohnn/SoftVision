...
#define GPU_MIN(a,b) std::min(a, b)
#define GPU_MAX(a,b) std::max(a, b)
#define GPU_CEIL(f)  std::ceil(f)
#define GPU_FLOOR(f) std::floor(f)
...
struct Range
{
     ...
     */
    inline unsigned int size() const { return end - begin; }
...
{
    return Range(GPU_MAX(a.begin, b.begin),
                 GPU_MIN(a.end, b.end));
}
...
struct ROI
{
     ...
     */
    inline unsigned int width()  const { return x.size(); }

    /**
     * @brief Get the ROI height
     * @return the Y range size
     */
    inline unsigned int height() const { return y.size(); }

    inline bool isEmpty() const { return x.isEmpty() || y.isEmpty(); }
     ...
     */
    inline bool contains(unsigned int in_x, unsigned int in_y) const
    {
 ...
 */
inline bool checkImageROI(const ROI& roi, int width, int height)
{
 ...
 */
inline Range downscaleRange(const Range& range, float downscale)
{
    return Range(GPU_FLOOR(range.begin / downscale), GPU_CEIL(range.end / downscale));
}
 ...
 */
inline Range upscaleRange(const Range& range, float upscale)
{
    return Range(GPU_FLOOR(range.begin * upscale), GPU_CEIL(range.end * upscale));
}
 ...
 */
inline Range inflateRange(const Range& range, float factor)
{
    const float midRange = range.begin + (range.size() * 0.5f);
    const float inflateSize = range.size() * factor * 0.5f;
    return Range(GPU_FLOOR(GPU_MAX(midRange - inflateSize, 0.f)), GPU_CEIL(midRange + inflateSize));
}
 ...
 */
inline ROI downscaleROI(const ROI& roi, float downscale)
{
 ...
 */
inline ROI upscaleROI(const ROI& roi, float upscale)
{
 ...
 */
inline ROI inflateROI(const ROI& roi, float factor)
{
