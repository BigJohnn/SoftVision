  ...
  REAL sigma, REAL eps)
{
  ...
  do {
    ...
    if (solver.info() != Eigen::Success) {
      LOG_INFO("error: decomposing linear system failed");
      return false;
    }
    x = solver.solve(AtF*b);
    if (solver.info() != Eigen::Success) {
      LOG_INFO("error: solving linear system failed");
      return false;
    }
    if (++iter > 32)
      break;
...
REAL RelRotationAvgError(const RelativeRotations& RelRs, const Matrix3x3Arr& Rs, REAL* pMin=nullptr, REAL* pMax=nullptr)
{
  ...
  for(int i=0; i < RelRs.size(); ++i) {
    ...
    acc(FrobeniusNorm(relR.Rij  - (Rs[relR.j]*Rs[relR.i].transpose())));
  }
  if (pMin)
  ...
  Eigen::Matrix<REAL,Eigen::Dynamic,1>& b)
{
  for (size_t r = 0; r < RelRs.size(); ++r) {
    ...
    const Vec3 erij;
    ceres::RotationMatrixToAngleAxis((const double*)eRij.data(), (double*)erij.data());
    b.block<3,1>(3*r,0) = Vec3(erij*relR.weight);
  }
}
  ...
  Matrix3x3Arr& Rs)
{
  for (size_t r = 0; r < Rs.size(); ++r) {
    ...
    Vec3 eRid = Vec3(x.block<3,1>(3*i,0));
    const Mat3 eRi;
  ...
  REAL sigma)
{
  ...
  do {
    ...
    if (!RobustRegressionL1PD(A, b, x)) {
      LOG_INFO("error: l1 robust regression failed.");
      return false;
    }
    ep = e; e = x.norm();
    if (ep < e)
  ...
  do {
    ...
    if (!IterativelyReweightedLeastSquares(A, b, x, sigma)) {
      LOG_INFO("error: l2 iterative regression failed");
      return false;
    }
    ep = e; e = x.norm();
    if (ep < e)
  ...
  LOG_X("Refine global rotations using L1RA-IRLS and " << nObss << " relative rotations:\n"
