...
struct CeresPairRotationError {
  CeresPairRotationError(const Vec3& relative_rotation,  const double weight)
  ...
  std::vector<Mat3> & vec_ApprRotMatrix)
{
  if (vec_relativeRot.empty() ||vec_ApprRotMatrix.empty() ) {
    LOG_DEBUG("Skip nonlinear rotation optimization, no sufficient data provided ");
    return false;
}

  // Convert global rotation to AngleAxis representation
  std::vector<Vec3> vec_Rot_AngleAxis(vec_ApprRotMatrix.size());
  ...
  for (size_t ii = 0; ii < vec_relativeRot.size(); ++ii)
  {
    ...
    const Mat3& Rrel = vec_relativeRot[ii].Rij;
